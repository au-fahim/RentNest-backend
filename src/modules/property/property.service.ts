...
export const createPropertyService = async (
  ...
  const isAvailable = payload.isAvailable !== undefined
    ? (payload.isAvailable === true || String(payload.isAvailable).toLowerCase() === "true")
    : true;
  ...
  const images = Array.isArray(payload.images) && payload.images.length > 0
    ? payload.images.map((img: any) => ({ url: img.url, publicId: img.publicId }))
    : undefined;
...
export const updatePropertyService = async (
  ...
  const imagesToCreate = Array.isArray(payload.images) && payload.images.length > 0
    ? payload.images.map((img: any) => ({ url: img.url, publicId: img.publicId }))
    : undefined;
...
export const deletePropertyService = async (
  ...
  const propertyImageClient = prisma as typeof prisma & {
    propertyImage: {
      findMany: (args: { where: { propertyId: string }; select?: { publicId: true } }) => Promise<Array<{ publicId: string }>>;
